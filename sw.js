const CACHE='bioolym7-v13-recovery-stable';
const CORE=[
  './',
  './index.html',
...
function inject(html){
  if(!html.includes('mobile-wow.css')){
    html=html.replace('</head>','<link rel="stylesheet" href="./mobile-wow.css"></head>');
  }
  if(!html.includes('app-pro.js')){
    html=html.replace(
      '</body>',
      '<script src="https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2"></script><script src="./config.js"></script><script src="./app-pro.js"></script></body>'
    );
  }
...
async function navigationResponse(req){
  try{
    ...
    return new Response(inject(text),{
      status:r.status,
